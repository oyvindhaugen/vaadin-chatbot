...
public class ChatView extends VerticalLayout {
    ...
    // here we introduce the Chat method as a final so it can be used later, but not changed
    private final ScheduledExecutorService executorService;
    // here we introduce the delay method that I mentioned earlier in MessageList.java


    public ChatView(Bot alice, ScheduledExecutorService executorService) {
        // This is the constructor for ChatView. This is so that every single method that gets constructed can be used.
        ...
        Button exit = new Button(VaadinIcon.EXIT.create(), event -> exitButton());
        exit.addClickShortcut(Key.ESCAPE);

        HorizontalLayout inputLayout = new HorizontalLayout(message, send, exit);
